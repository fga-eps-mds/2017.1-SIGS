...
#Class is responsable to read csv and save in database
class Parser < ApplicationRecord
  ...
  def self.save_data_buildings(file)
    CSV.foreach(file, col_sep: ',', headers: true, encoding: 'Windows-1252') do |row|
      build_record = Building.find_or_create_by(code: row[0], name: row[1], wing: row[2])
  ...
  def self.save_data_departments(file)
    CSV.foreach(file, col_sep: ',', headers: true, encoding: 'Windows-1252') do |row|
      row2_data = row[2]
      departament_record = Department.find_or_create_by(code: row[0], acronym: row2_data,
        name: row2_data)
