...
RSpec.describe SchoolRoomsController, type: :controller do
  ...
  describe 'SchoolRooms methods' do
    ...
    before(:each) do
      ...
      @school_room = SchoolRoom.create(name:"YY", capacity: 50, discipline: @discipline1, course_ids: [@course1.id, @course3.id])
      sign_in(@user)
    end
    ...
    it 'not should create school room with high capacity' do
      post :create, params:{school_room: {name: 'AA',  capacity: 800, discipline: Discipline.last, course: [@course1.id, @course3.id]}}
    ...
    it 'not should delete school room because user not have permission' do
      school_room = SchoolRoom.create(name: 'AA',capacity: 50, discipline_id: @discipline3.id, course_ids: [@course1.id, @course3.id])
      get :destroy, params:{id: school_room.id}
